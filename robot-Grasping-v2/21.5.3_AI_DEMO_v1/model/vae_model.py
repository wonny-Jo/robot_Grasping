...
class Encoder(nn.Module):
    def __init__(self, use_inception, repeat_per_module):
        super().__init__()
        # stages
        self.upch1 = nn.Conv2d(3, 32, kernel_size=1)
...
class Decoder(nn.Module):
    def __init__(self, use_inception, repeat_per_module):
        ...
        self.last = nn.ConvTranspose2d(32, 3, kernel_size=1)
...
class VAE(nn.Module):
    def __init__(self, use_inception, repeat_per_block, device):
        ...
        # Middle
        self.fc_mu = nn.Linear(sz_mid, self.n_latent_features)
        self.fc_logvar = nn.Linear(sz_mid, self.n_latent_features)
        self.fc_rep = nn.Linear(self.n_latent_features, sz_mid)
        # Decoder
    ...
    def forward(self, x):
        ...
        BCE = F.binary_cross_entropy(d, x, size_average=False)
        KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        loss = BCE + KLD
        return d, mu, logvar, loss.mean()
