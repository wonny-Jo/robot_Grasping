...
class URRobot(object):
    ...
    def __init__(self, host, use_rt=False):
        ...
        self.fthost = host
        self.ftmon = None
        self.ftmon = self.get_ftsensor_monitor()
    ...
    def get_ftsensor_monitor(self):
        if not self.ftmon:
            self.ftmon = robotiq_fts.FTSensorMonitor(self.fthost)
            self.ftmon.start()
        self.ftmon.set_csys(self.csys)
        return self.ftmon

    def get_fts_data(self):
        return self.ftmon.getFTForce()